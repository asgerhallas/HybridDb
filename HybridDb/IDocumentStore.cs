...
namespace HybridDb
{
    public interface IDocumentStore : IDisposable
    {
        ...
        IEnumerable<IDictionary<IColumn, object>> Query(ITable table, out QueryStats stats, string columns = "*", string @where = "", int skip = 0, int take = 0,
                                                        string orderby = "", object parameters = null);

        IEnumerable<TProjection> Query<TProjection>(ITable table, out QueryStats stats, string columns = "*", string @where = "", int skip = 0, int take = 0, string @orderby = "", object parameters = null);
    }
}