...
{
    public class EventStoreTests(ITestOutputHelper output) : HybridDbTests(output)
    {
        ...
        protected static EventData<byte[]> CreateEventData(string streamId, long sequenceNumber, Guid eventId, string name = "myevent") =>
            new EventData<byte[]>(streamId, eventId, name, sequenceNumber, new Metadata(), [(byte) sequenceNumber]);
    }
}