...
namespace HybridDb.Tests
{
	public class DocumentStoreTests : HybridDbTests
	{
		...
		[Fact]
		public void CanRawQueryProjectToNestedProperty()
		{
			Document<Entity>().With(x => x.TheChild.NestedDouble);
			...
			var rows = store.RawQuery<ProjectionWithNestedProperty>($"select * from {tableName}", out _).ToList();
		...
		[Fact]
		public void CanRawQueryAndReturnFullDocuments()
		{
			Document<Entity>().With(x => x.Field);
			...
			var tableName = store.Database.FormatTableNameAndEscape(table.Table.Name);
			var rows = store.RawQuery($"select * from {tableName} where Field != @name", out _, parameters: new { name = "Bjarne" }).ToList();
		...
		[Fact]
		public void CanRawQueryAndReturnAnonymousProjections()
		{
			Document<Entity>().With(x => x.Field);
			...
			IEnumerable<dynamic> Query<T1>(T1 prototype) =>
				store.RawQuery<T1>($@"select Field from {tableName} where Field = @name", out _, new { name = "Asger" }).Cast<dynamic>();
		...
		[Fact]
		public void CanRawQueryAndReturnValueProjections()
		{
			Document<Entity>().With(x => x.Field);
			...
			var tableName = store.Database.FormatTableNameAndEscape(table.Table.Name);
			var rows = store.RawQuery<string>($"select Field from {tableName}", out _).ToList();
		...
		[Fact]
		public void CanRawQueryDynamicTable()
		{
			Document<Entity>().With(x => x.Field).With(x => x.Property);
			...
			var tableName = store.Database.FormatTableNameAndEscape(table.Table.Name);
			var rows = store.RawQuery($"select * from {tableName} where Field = @name", out _, new { name = "Asger" }).ToList();
