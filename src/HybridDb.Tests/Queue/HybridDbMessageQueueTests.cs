...
namespace HybridDb.Tests.Queue
{
    public class HybridDbMessageQueueTests : HybridDbTests
    {
        ...
        public async Task DequeueAndHandle_ByOrder()
        {
            ...
            session.Enqueue(new MyMessage("order-2.1"), null, null, 2);
            session.Enqueue(new MyMessage("order-2.2"), null, null, 2);
            session.Enqueue(new MyMessage("order-1.1"), null, null, 1);
            session.Enqueue(new MyMessage("order-1.2"), null, null, 1);
